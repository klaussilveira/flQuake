...
char *PR_UglyValueString(etype_t type, eval_t *val)
{
    ...
    case ev_void:
        ...
        break;
    case ev_float:
        sprintf(line, "%s", MK_cleanftos(val->_float));  // reduced savegames
        break;
    case ev_vector:
        // reduced savegames - begin
...
====================
*/
char		prev_ed_name[256]; // Edited
char		badfields[256*4]; // Edited
...
//
        ...
        if (!func) {
            ...
            if (Q_strcmp(prev_ed_name, PR_ValueString(d->type, (eval_t *)v))) {
                ...
                ED_Print(ent);
                //		if (badfields[0]) Con_Printf(badfields); // Edited
            } // Edited
            ED_Free(ent);
            continue;
        }
        if (badfields[0]) {
            Con_Printf(badfields);    // Edited
        }
