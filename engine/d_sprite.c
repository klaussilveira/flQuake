...
void D_SpriteDrawSpans_A(sspan_t *pspan,float alpha) // transparencies
{
    int			count, spancount, izistep;
    int			izi;
    pixel_t		*pbase, *pdest,btemp;
            ...
            do {
                ...
                if (btemp != 0xdeadbeaf)
                    if (*pz <= (izi >> 16)) {
                        int R,G,B,C;
                        C	=	*pdest;
                        R	=	((C&0xff)*iAlpha)+((btemp&0xff)*Alpha);
                        G	=	((((C>>8)&0xff)*iAlpha)+(((btemp>>8)&0xff)*Alpha))<<8;
                        B	=	((((C>>16)&0xff)*iAlpha)+(((btemp>>16)&0xff)*Alpha))<<16;
                        *pdest = R|G|B;
                        //*pdest = alphamap[btemp + *pdest*256]; // transparencies
                    }
...
void D_SpriteDrawSpans_Stippled(sspan_t *pspan)  // transparencies
{
    int			count, spancount, izistep;
    int			izi;
    pixel_t		*pbase, *pdest,btemp;
...
// we count on FP exceptions being turned off to avoid range problems
    ...
    do {
        pdest = d_viewbuffer + (screenwidth * pspan->v) + pspan->u;
        pz = d_pzbuffer + (d_zwidth * pspan->v) + pspan->u;
        teste = (((int)pdest-(int)d_viewbuffer) / screenwidth) & 1;  // stipple alpha
        ...
        do {
            ...
            do {
                ...
                if (btemp != 0xdeadbeaf) {
                    if (*pz <= (izi >> 16)) {
                        if (((int)pdest + teste) & 1) { // stipple alpha
                            // stipple alpha
                            *pz = izi >> 16;
                            *pdest = btemp;
                        } // stipple alpha
...
void D_SpriteDrawSpans_Add(sspan_t *pspan)  // transparencies
{
    int			count, spancount, izistep;
    int			izi;
    pixel_t		*pbase, *pdest,btemp;
...
#define SPRITE_ADD(X,Y)
                ...
                if (btemp != 0xdeadbeaf)
                    if (*pz <= (izi >> 16)) {
                        ...
                        C	=	*pdest;
                        R	=	(btemp&0xff)+(C&0xff);
                        G	=	(btemp&0xff00)+(C&0xff00);
                        B	=	(btemp&0xff0000)+(C&0xff0000);
                        if (R>0xff) {
                            R=0xff;
                        }
                        if (G>0xff00) {
                        ...
                        *pdest	=	R|G|B;
//                        *pdest = additivemap[btemp + *pdest*256]; // transparencies
                    }
...
void D_SpriteDrawSpans_Shadow(sspan_t *pspan)  // transparencies
{
    int			count, spancount, izistep;
    int			izi;
    pixel_t		*pbase, *pdest,btemp;
        ...
        do {
            ...
            do {
                ...
                if (btemp != 0xdeadbeaf)
                    // begin
                {
                    intensity = (1.f - ((float)((izi >> 16)- *pz)/20.f));
                    if (intensity == 1 && currententity->alpha == 1) {
                        *pdest = 0;
                    } else if (intensity > 0 && intensity <= 1) {
                        *pdest = (byte)vid.colormap[*pdest + ((32 + (int)(31.0*intensity*currententity->alpha)) * 256)];
                    }
                }
                // end
...
void D_SpriteCalculateGradients(void)
{
    ...
    distinv = 1.f / (-DotProduct(modelorg, r_spritedesc.vpn));
...
=====================
*/
extern cvar_t sw_stipplealpha, r_sprite_addblend; // Edited
