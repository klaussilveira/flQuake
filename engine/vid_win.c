...
static int		vid_stretched;//, windowed_mouse; // removed
...
#define MODE_FULLSCREEN_DEFAULT	(MODE_WINDOWED + 7)

// Note that 0 is MODE_WINDOWED
cvar_t		vid_mode = {"vid_mode","0", false};
// Note that 0 is MODE_WINDOWED
cvar_t		_vid_default_mode = {"_vid_default_mode","0", true};
// Note that 7 is MODE_FULLSCREEN_DEFAULT //Edited was 3
cvar_t		_vid_default_mode_win = {"_vid_default_mode_win","7", true};
...
//cvar_t		_windowed_mouse = {"_windowed_mouse","0", true}; // removed
cvar_t		vid_fullscreen_mode = {"vid_fullscreen_mode","3", true};
cvar_t		vid_windowed_mode = {"vid_windowed_mode","1", true};
...
HDC			g_hMemDC	=	0;
HBITMAP		g_hBmp		=	0;
unsigned int* g_pBufferC	=	0;
pixel_t* g_pBufferB	=	0;
...
//    HDC				hdc;
    ...
    wc.lpszMenuName  = 0;
    wc.lpszClassName = "Makaqu"; // WinQuake // edited
...
char *VID_GetModeDescription2(int mode)
{
    ...
    } else if (modelist[mode].type == MS_WINDOWED) {
        sprintf(pinfo,"%s fullscreen", pv->modedesc);
    } else {
...
// empty while Quake starts up.
    ...
    if (!g_hMemDC) {
        ...
        g_hBmp = CreateDIBSection(g_hMemDC, &bmi, DIB_RGB_COLORS, (void**)&g_pBufferC, NULL, 0);
        free(g_pBufferB);
        g_pBufferB	=	malloc(DIBWidth*DIBHeight*sizeof(pixel_t));
    ...
    */
    memset(g_pBufferB,0xff,DIBWidth*DIBHeight*sizeof(pixel_t));
    vid.buffer = vid.conbuffer = vid.direct = g_pBufferB;
    vid.rowbytes = vid.conrowbytes = DIBWidth*sizeof(pixel_t);
    vid.numpages = 1;
//	vid.maxwarpwidth = WARP_WIDTH; // hi-res waterwarp - removed
//	vid.maxwarpheight = WARP_HEIGHT; // hi-res waterwarp - removed
    vid.height = vid.conheight = DIBHeight;
    vid.width = vid.conwidth = DIBWidth;
    vid.maxwarpwidth = vid.width; // hi-res waterwarp
    vid.maxwarpheight = vid.height; // hi-res waterwarp
    /*	// buffered video - begin
    	vid_card_buffer = dibdc->surface;
    	if (vid_buffer)
    	{
    		free(vid_buffer);
    		vid_buffer = NULL;
    	}
    	vid_buffer = (byte  *)malloc((int)(vid.rowbytes*vid.height));
...
qboolean VID_SetFullscreenMode(int modenum)
{
    ...
    //	vid_card_buffer = NULL;
        // buffered video - end
        vid.buffer = vid.conbuffer = vid.direct = NULL; // buffered video - removed
    //	vid.maxwarpwidth = WARP_WIDTH; // hi-res waterwarp - removed
    //	vid.maxwarpheight = WARP_HEIGHT; // hi-res waterwarp - removed
...
// position and show the DIB window
    ...
    */
    ...
    vid.buffer = vid.conbuffer = vid.direct = g_pBufferB; // buffered video - removed
    vid.rowbytes = vid.conrowbytes = WindowRect.right*sizeof(pixel_t);
    vid.numpages = 1;
//	vid.maxwarpwidth = WARP_WIDTH; // hi-res waterwarp - removed
//	vid.maxwarpheight = WARP_HEIGHT; // hi-res waterwarp - removed
    vid.height = vid.conheight = DIBHeight;
...
// so Con_Printfs don't mess us up by forcing vid and snd updates
    ...
    if (modelist[modenum].type == MS_WINDOWED) {
        if (key_dest != key_menu && !cl.paused) { // edited
    ...
    } else if (modelist[modenum].type == MS_WINDOWED) {
...
void VID_LockBuffer(void)
{
    /*
        ...
        if (memdc)
        {
            ...
            vid.buffer = vid.conbuffer = vid.direct = memdc->surface; // buffered video - removed
            //	vid_card_buffer = memdc->surface; // buffered video
            vid.rowbytes = vid.conrowbytes = memdc->mi.bytesPerLine;
        }
        else if (mgldc)
        {
            // Update surface pointer for linear access modes
            vid.buffer = vid.conbuffer = vid.direct = mgldc->surface; // buffered video - removed
            //	vid_card_buffer = mgldc->surface; // buffered video
            vid.rowbytes = vid.conrowbytes = mgldc->mi.bytesPerLine;
        }
        // buffered video - begin
    //	vid.buffer = vid.conbuffer = vid.direct = vid_buffer;
    //	vid.buffer = vid.conbuffer = vid.direct = vid_buffer = vid_card_buffer;
    //	vid.buffer = vid.conbuffer = vid.direct = vid_card_buffer;
        // buffered video - end

    //	if (r_dowarp) // buffered video (bloody hack) - removed
        d_viewbuffer = r_warpbuffer;
    //	else // buffered video (bloody hack) - removed
    //		d_viewbuffer = (void *)(byte *)vid.buffer; // buffered video (bloody hack) - removed
    //		d_viewbuffer = vid_buffer; // não funciona
    //	Con_Printf("r_warpbuffer %u\n", &r_warpbuffer);
    //	Con_Printf("vid_buffer %u\n", &vid_buffer);

    //	if (r_dowarp) // buffered video (bloody hack) - removed
        screenwidth = vid.width;//WARP_WIDTH; // hi-res waterwarp - edited
    //	else // buffered video (bloody hack) - removed
    //		screenwidth = vid.rowbytes; // buffered video (bloody hack) - removed
...
void VID_UnlockBuffer(void)
{
    ...
    //	vid_card_buffer = NULL; // buffered video
        vid.buffer = vid.conbuffer = vid.direct = d_viewbuffer = NULL; // buffered video - removed
    */
}
...
//	Cvar_RegisterVariable (&_windowed_mouse); // removed
...
// fullscreen DIBs as well
    ...
    vid.maxwarpwidth = 320;//WARP_WIDTH; // hi-res waterwarp - edited
    vid.maxwarpheight = 200;//WARP_HEIGHT; // hi-res waterwarp - edited
...
void FlipScreen(vrect_t *rects)
{
    ...
    hdc = GetDC(mainwindow);//BeginPaint(mainwindow, &ps);

    for (y=0; y<DIBHeight; y++)
        for (x=0; x<DIBWidth; x++) {
            a	=	g_pBufferB[x+(DIBHeight-y-1)*DIBWidth];
            g_pBufferC[x+y*DIBWidth]=a;
        }

    //for(y=0;y<DIBHeight;y++)
    //for(x=DIBWidth-1;x>=0;x--)
    //	g_pBuffer[x+y*DIBWidth]	=	((byte*)g_pBuffer)[x+y*DIBWidth];


    BitBlt(hdc,0,0,DIBWidth,DIBHeight,g_hMemDC,0,0,SRCCOPY);
...
void	VID_Update(vrect_t *rects)
{
    ...
    //	VID_LockBuffer();
    	...
    	if (vid_card_buffer && vid_buffer)
    	{
    	int		i;
    	byte	*src;
    	byte	*dest;
    	src = vid_buffer;
    	dest = vid_card_buffer;
    //	dest = vid_card_buffer + scr_vrect.y * vid.rowbytes + scr_vrect.x;
...
// handle the mouse state when windowed if that's changed
    if (modestate == MS_WINDOWED) {
//		if (_windowed_mouse != windowed_mouse) // edited
        {
            if (key_dest != key_menu && !cl.paused) { // edited
                IN_ActivateMouse();
                IN_HideMouse();
...
//			windowed_mouse = _windowed_mouse; // edited
        }
    }
}
    ...
    if (vid.numpages == 1) {
...
#if 0
        {
            ...
            for (i=0 ; i<(height << repshift) ; i += reps)
                for (j=0 ; j<width ; j++) {
                    t = pbitmap[(i >> repshift) * width + j];
                    if (t != TRANSPARENT_COLOR) {
                        backingbuf[i*24 + j] = vid.direct[((y<<repshift)+i)*(width*vid.rowbytes) + x+j];
                        vid.direct[((y<<repshift)+i)*(width*vid.rowbytes) + x+j] = t;
                    }
                }
        }
#else
        ...
        {
            byte *pdest, t;
            pdest = (byte *)mgldc->surface;
            for (i=0 ; i<(height << repshift) ; i += reps)
                for (j=0 ; j<width ; j++) {
                    t = pbitmap[(i >> repshift) * width + j];
                    if (t != TRANSPARENT_COLOR) {
                        backingbuf[i*24 + j] = pdest[((y<<repshift)+i)*(width*mgldc->mi.bytesPerLine) + x+j];
                        pdest[((y<<repshift)+i)*(width*mgldc->mi.bytesPerLine) + x+j] = t;
                    }
                }
        }
#else
    ...
    // minimize/restore fulldib windows/mouse-capture normal windows on demand
        if (!in_mode_set)
        {
            if (ActiveApp)
            {
                ...
                else if ((modestate == MS_WINDOWED) && key_dest != key_menu && !cl.paused) // edited
                {
                    IN_ActivateMouse ();
                    IN_HideMouse ();
                }
...
void VID_HandlePause(qboolean pause)
{
    ...
    if ((modestate == MS_WINDOWED)) // edited
        if (key_dest != key_menu) { // Edited
            if (pause) {
                IN_DeactivateMouse();
                IN_ShowMouse();
            } else {
...
extern void M_Video_f(void);  // edited
...
void VID_MenuDraw(void)
{
    ...
    M_DrawTransPic((320-p->width)/2, 4, p);   // edited
