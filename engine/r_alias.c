...
// model interpolation - begin
trivertx_t		*r_apverts1;
trivertx_t		*r_apverts2;
trivertx_t		*bboxmin1, *bboxmax1, *bboxmin2, *bboxmax2; // Edited
float			blend;
// model interpolation - end
...
// construct the base bounding box for this frame
        /*	// model interpolation - removed - begin
        ...
        */	// model interpolation - removed - end
        pframedesc = &pahdr->frames[currententity->frame]; // model interpolation - edited
...
// TODO: probably should do this loop in ASM, especially if we use floats
    ...
    if (!r_interpolation.value) // model interpolation
// begin
#if	id386
        if (!use_c_renderer)
#endif
// end
        {
            // model interpolation - the engine is crashing when both trivial_accept and interpolation are used
...
void R_AliasPreparePoints(void)
{
    ...
    for (i=0 ; i<r_anumverts ; i++, fv++, av++, r_apverts++, r_apverts1++, r_apverts2++, pstverts++) { // model interpolation - edited
        // model interpolation - begin
        if (r_interpolation.value) {
...
// TODO: could cache lazily, stored in the entity
    ...
    if (!r_interpolation.value || (currententity == &cl.viewent) || currententity == &cl_entities[cl.viewentity]) { // model interpolation
        // model interpolation
...
{
    vec3_t	v, v1;
    float	lightcos1, lightcos2; // Edited
    int		lightcos; // Edited
...
================
*/
void R_AliasSetupLighting(/*alight_t *plighting*/)  // edited
{
    // Model interpolation - begin
...
void R_AliasSetupFrame(void)
{
    ...
    if (paliashdr->frames[frame].type == ALIAS_SINGLE) {
        paliasgroup = NULL; // model interpolation
        pose = frame; // model interpolation
        r_apverts = (trivertx_t *)
                    ((byte *)paliashdr + paliashdr->frames[frame].frame);
        //	return; // model interpolation - removed
    } else { // model interpolation
        // model interpolation
...
//
    ...
    currententity->framegroup1 = currententity->framegroup2 = paliasgroup; // edited
...
void R_AliasSetupBlendedFrame(void)
{
    ...
    if (e->lastmodel == e->model && interval) { // && (e->framegroup2 == paliasgroup)) // edited
        if ((pose != e->pose2) || (paliasgroup != e->framegroup2)) { // edited
            if (e->reset_frame_interpolation == true) {
                e->framegroup1 = paliasgroup; // Edited
                e->pose1 = pose;
                e->reset_frame_interpolation = false;
            } else {
                e->framegroup1 = e->framegroup2; // Edited
                e->pose1 = e->pose2;
            }
            e->framegroup2 = paliasgroup; // Edited
            e->pose2 = pose;

            e->frame_start_time = (float)cl.time;
            blend = 0;
        } else {
            blend = ((float)cl.time - e->frame_start_time) / interval;    // edited
        }
    } else {
        e->pose1 = e->pose2 = pose;
        e->lastmodel = e->model;
        e->frame_start_time = (float)cl.time;
        e->framegroup1 = e->framegroup2 = paliasgroup; // edited
        blend = 0;
    }

    if (blend > 1.0f) { // edited
        blend = 1.0f;
    }

    // FIXME ?
    else if (blend < 0) { // edited
...
================
*/
void R_AliasDrawModel(/* alight_t *plighting */)  // edited
{
...
// cache align
    ...
    if (currententity->effects & EF_FULLBRIGHT) {
        ...
        // end
    {
        R_AliasSetupLighting(/* plighting */);    // edited
    }
