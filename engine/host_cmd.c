...
{
    /* // "quit" always quits - removed - begin
    if (key_dest != key_console && cls.state != ca_dedicated)
    {
    	M_Menu_Quit_f ();
    	return;
    }
    */ // "quit" always quits - removed - end
...
==================
*/
extern cvar_t sv_qcexec; // Edited
void Host_QC_Exec(void)
{
    ...
    if (!(sv_qcexec.value || developer.value)) { // Edited
        return;
    }
...
//
//======================================
char *MK_cleanftos(float f);  // reduced savegames
...
// VMU saves - end
...
#else
    f = fopen(name, "wb");  // reduced savegames - edited
#endif
    ...
    for (i=0 ; i<NUM_SPAWN_PARMS ; i++) {
        fprintf(f, "%s\n", MK_cleanftos(svs.clients->spawn_parms[i]));    // reduced savegames
    }
    fprintf(f, "%d\n", current_skill);
    fprintf(f, "%s\n", sv.name);
...
// VMU saves - end
    sprintf(name, "%s/%s", com_gamedir, Cmd_Argv(1));
#endif // VMU saves
    COM_DefaultExtension(name, ".SAV");  // upper-case savegame filenames - edited
    ...
    fprintf(f, "%s\n", MK_cleanftos(sv.time));  // reduced savegames
//#ifdef _arch_dreamcast // Edited
    fprintf(f, "%i\n", svs.serverflags);  // serverflags bugfix
//#endif // Edited
...
// VMU saves - end
        {
            Host_Error("First token isn't a brace");  // edited
            return; // Edited
        }
#endif // VMU saves
...
void Host_Pause_f(void)
{
    ...
    if (!pausable.value) {
        ...
        if (svs.maxclients > 1 || key_dest != key_game) { // Edited
            // Edited
            if (sv.paused) {
...
//	for (i=1 ; i<c+1 ; i++) // removed
//		strncpy (cls.demos[i-1], Cmd_Argv(i), sizeof(cls.demos[0])-1); // removed
...
void Host_InitCommands(void)
{
    ...
    Cmd_AddCommand("loadsmall", Host_SmallLoadgame_f);  // small saves
    Cmd_AddCommand("savesmall", Host_SmallSavegame_f);  // small saves
