...
cvar_t	scr_ofsy = {"scr_ofsy","0", true}; // saved in the config file - edited
cvar_t	scr_ofsz = {"scr_ofsz","0", false};

cvar_t	v_deathtilt = {"v_deathtilt", "1"}; // Edited
cvar_t	v_deathtiltangle = {"v_deathtiltangle", "80"}; // Edited
...
cvar_t	gl_polyblend = {"gl_polyblend", "1", false}; // gl_polyblend
int	gl_polyblend_old; // gl_polyblend

cvar_t	cl_nobob = {"cl_nobob","0", true}; // cl_nobob
...
cvar_t	v_centerspeed = {"v_centerspeed","200"}; // 500 // edited
...
void V_UpdatePalette(void)
{
    ...
    return;	//do don't support changing paletts on runtime as the textures are converted to true color during loading
    //not returning here causes the red-level bug after the death in lava
...
// gl_polyblend begin
    ...
    for (i=0 ; i<256 ; i++) {
        ...
        if (gl_polyblend.value != 0) // gl_polyblend
            for (j=0 ; j<NUM_CSHIFTS ; j++) {
...
//			Con_Printf("%f\n", dmg);
    ...
    if (cl.stats[STAT_HEALTH] <= 0 && !chase_active.value && v_deathtilt.value) { // edited
...
extern cvar_t r_stereo_separation; // stereo 3D
extern int		r_stereo_side; // stereo 3D
...
//		r_refdef.vrect.height >>= (r_framecount&1); // Edited
...
void V_Init(void)
{
    ...
    Cvar_RegisterVariable(&gl_polyblend);  // gl_polyblend
    Cvar_RegisterVariable(&cl_nobob);  // cl_nobob
    Cvar_RegisterVariable(&v_deathtilt);  // Edited
    Cvar_RegisterVariable(&v_deathtiltangle);  // Edited
