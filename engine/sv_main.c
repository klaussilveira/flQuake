...
extern int current_protocol; // Edited
...
cvar_t sv_makaqu = {"sv_makaqu", "0"}; // Edited
cvar_t sv_compatibility = {"sv_compatibility", "0"}; // Edited
cvar_t sv_qcexec = {"sv_qcexec","0", false, true}; // qcexec
/*
===============
SV_Init
===============
...
void SV_Init(void)
{
    ...
    extern	cvar_t	sv_aim_h; // horizontal autoaim
    extern	cvar_t	sv_aim;
    extern	cvar_t	sv_enable_use_button; // +USE fix
    ...
    Cvar_RegisterVariable(&sv_aim_h);  // horizontal autoaim
    Cvar_RegisterVariable(&sv_aim);
    Cvar_RegisterVariable(&sv_nostep);
    Cvar_RegisterVariable(&sv_enable_use_button);  // +USE fix
    Cvar_RegisterVariable(&sv_makaqu);  // Edited
    Cvar_RegisterVariable(&sv_compatibility);  // Edited
    Cvar_RegisterVariable(&sv_qcexec);  // qcexec
...
// directed messages go only to the entity the are targeted on
    ...
    if (current_protocol == QB_PROTOCOL) { //Edited
...
void SV_SendServerinfo(client_t *client)
{
    ...
    if (sv_compatibility.value) { // Edited
        MSG_WriteLong(&client->message, PROTOCOL_VERSION);
    } else { // Edited
        MSG_WriteLong(&client->message, QB_PROTOCOL);    // 16-bit angles - edited
    }
...
//	float	glow_blue;
    ...
    float	frame_interval=0.1f; // QC frame_interval
...
// send an update
        ...
        if (ent->baseline.effects != ent->v.effects) {
            ...
            bits |= U_EFFECTS;
            // begin
            if (current_protocol == QB_PROTOCOL)
...
void SV_WriteClientdataToMessage(edict_t *ent, sizebuf_t *msg)
{
    ...
    eval_t	*val;
#endif
    extern int current_protocol; // high values in the status bar
...
// [always sent]	if (bits & SU_ITEMS)
    ...
    // high values in the status bar - begin
    if (current_protocol == QB_PROTOCOL) {
        ...
        // high values in the status bar - end
        MSG_WriteByte(msg, ent->v.currentammo);
        MSG_WriteByte(msg, ent->v.ammo_shells);
        MSG_WriteByte(msg, ent->v.ammo_nails);
        MSG_WriteByte(msg, ent->v.ammo_rockets);
        MSG_WriteByte(msg, ent->v.ammo_cells);
    } // high values in the status bar
...
#endif
{
    edict_t		*ent;
    int			i;
    extern int current_protocol; // 16-bit angles
