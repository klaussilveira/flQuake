...
void R_BuildLightMap(void)
{
    ...
    tmax = (surf->extents[1]>>4)+1;
    size = smax*tmax;
    lightmap = cl.worldmodel->lightdata+(surf->samples-cl.worldmodel->lightdata)/LightmapScaledown;
...
// << 16 components are to guarantee positive values for %
    ...
    soffset	=	soffset/LightmapScaledown;
...
#if	!id386
#define MUL(X,Y,B)  ((((X>>B)&0xff)*Y)>>15)
#define MIX     pix  = psource[b]; \
				ilight	=	(0xffff-(light&0xffff)); \
				ilight  =	(ilight*ilight)>>24; \
				ilight  =	(ilight*ilight*ilight)>>8; \
				R	=	MUL(pix,ilight,0); \
				G	=	MUL(pix,ilight,8); \
				B	=	MUL(pix,ilight,16); \
                prowdest[b] = (R<0?0:R>255?255:R)|((G<0?0:G>255?255:G)<<8)|((B<0?0:B>255?255:B)<<16); \
                light += lightstep;
...
void R_DrawSurfaceBlock8_mip0(void)
{
    int				v, i, b, lightstep, lighttemp;
    unsigned int	light, ilight;
