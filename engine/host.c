...
cvar_t	r_skyalpha = {"r_skyalpha","0"}; //0.6 translucent sky
...
// Deathmatch/Coop not at the same time - begin
...
// Deathmatch/Coop not at the same time - end
// TIMESCALE extension - begin
...
// 2001-10-20 TIMESCALE extension by Tomaz/Maddes  start
    Cvar_RegisterVariableWithCallback(&host_timescale, Host_CheckTimescaleValue);  // Edited
// 2001-10-20 TIMESCALE extension by Tomaz/Maddes  end
    Cvar_RegisterVariable(&r_skyalpha);  // translucent sky
    ...
    Cvar_RegisterVariableWithCallback(&deathmatch, Host_CheckDeathmatchValue);  // Deathmatch/Coop not at the same time - edited
    Cvar_RegisterVariableWithCallback(&coop, Host_CheckCoopValue);  // Deathmatch/Coop not at the same time - edited
...
#ifndef _arch_dreamcast // VMU saves
        //	f = fopen (va("%s/config.cfg",com_gamedir), "wb"); // reduced config file - edited

#ifdef FLASH
        f = as3OpenWriteFile(va("%s/makaqu.cfg",com_gamedir));
#else
        f = fopen(va("%s/makaqu.cfg",com_gamedir), "wb");  // config.cfg replacement - edited
#endif
