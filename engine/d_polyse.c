...
void D_DrawNonSubdiv_C(void);  // transparencies

//#if	!id386 // transparencies - removed
...
================
*/
void D_PolysetDraw_C(void)  // transparencies - edited
{
    ...
    if (r_affinetridesc.drawtype) {
        ...
        D_DrawNonSubdiv_C();  // transparencies - edited
    }
}

#if	!id386 // transparencies
...
================
*/
void D_DrawNonSubdiv_C(void)  // transparencies - edited
{
    mtriangle_t		*ptri;
    finalvert_t		*pfv, *index0, *index1, *index2;
    int				i;
...
====================
*/
void D_PolysetScanLeftEdge_C(int height)  // transparencies - edited
{
...
//#endif	// !id386 // transparencies - removed
...
================
*/
void D_PolysetCalcGradients_C(int skinwidth)  // transparencies - edited
{
    ...
    xstepdenominv = 1.f / (float)d_xdenom;
...
#if	0//id386 // transparencies - edited
...
void D_PolysetDrawSpans8(spanpackage_t *pspanpackage)
{
    ...
    do {
        ...
        if (lcount) {
            ...
            do {
                if ((lzi >> 16) >= *lpz)
                    if (*lptex != 0xdeadbeaf) { // transparent pixels in alias models
#ifdef BILINEAR
                        ...
                        c00	=	*(lptex);
                        c01	=	*(lptex + 1);
                        c10	=	*(lptex    +  r_affinetridesc.skinwidth);
                        c11	=	*(lptex + 1+  r_affinetridesc.skinwidth);
                        ...
                        *lpdest = LRP(c00,c10,ym,ymi,0)|LRP(c00,c10,ym,ymi,8)|LRP(c00,c10,ym,ymi,16);
#else
                        *lpdest = *lptex;
#endif
                        *lpz = lzi >> 16;
...
#endif	// !id386
// 66% transparency
void D_PolysetDrawSpans8_66(spanpackage_t *pspanpackage)  // transparencies - edited
{
    int		lcount;
    pixel_t	*lpdest,*lptex;
    int		lsfrac, ltfrac;
    ...
    do {
        ...
        if (lcount) {
            ...
            do {
                if ((lzi >> 16) >= *lpz)
                    if (*lptex != 0xdeadbeaf) { // transparent pixels in alias models
                        // transparencies - begin
//#define temp ((byte *)acolormap)[*lptex + (llight & 0xFF00)]
//                        *lpdest = alphamap[*lpdest + temp*256];
                        // transparencies - end
#define LRPL(X,Y,L,IL,B)  ((((((X>>B)&0xff)*IL)>>16)+((((Y>>B)&0xff)*L)>>8))<<B)
                        *lpdest = LRPL(*lpdest,*lptex,170,86,0)|LRPL(*lpdest,*lptex,170,86,8)|LRPL(*lpdest,*lptex,170,86,16);
#undef LRPL
...
// 33% transparency
void D_PolysetDrawSpans8_33(spanpackage_t *pspanpackage)  // transparencies - edited
{
    int		lcount;
    pixel_t	*lpdest;
    pixel_t	*lptex;
...
//                        *lpdest = alphamap[temp + *lpdest*256];
                        // transparencies - end
#define LRPL(X,Y,L,IL,B)  ((((((X>>B)&0xff)*IL)>>16)+((((Y>>B)&0xff)*L)>>8))<<B)
                        *lpdest = LRPL(*lpdest,*lptex,86,170,0)|LRPL(*lpdest,*lptex,86,170,8)|LRPL(*lpdest,*lptex,86,170,16);
#undef LRPL
                    }
                lpdest++;
                lzi += r_zistepx;
...
// stippled alpha (50%)
void D_PolysetDrawSpans8_Stippled(spanpackage_t *pspanpackage)  // transparencies - edited
{
    int		lcount;
    pixel_t	*lpdest;
    pixel_t	*lptex;
    ...
    do {
        ...
        if (lcount) {
            int		teste = (((int)pspanpackage->pdest-(int)d_viewbuffer) / screenwidth) & 1;  // stipple alpha
            lpdest = pspanpackage->pdest;
            lptex = pspanpackage->ptex;
            lpz = pspanpackage->pz;
            lsfrac = pspanpackage->sfrac;
...
// additive blending
void D_PolysetDrawSpans8_Add(spanpackage_t *pspanpackage)  // transparencies - edited
{
    int		lcount;
    pixel_t	*lpdest;
    pixel_t	*lptex;
...
// shadow blending
void D_PolysetDrawSpans8_Shadow(spanpackage_t *pspanpackage)  // EF_SHADOW - edited
{
    int		lcount;
    byte	*lpdest;
    int		lzi;
...
//					*lpdest = 0;
                {
                    *lpdest = (byte)vid.colormap[*lpdest + ((32 + (int)(31.0*currententity->alpha)) * 256)];
                }
                // end
                lpdest++;
                lzi += r_zistepx;
...
void D_PolysetDrawSpans8_Metal(spanpackage_t *pspanpackage)
{
    ...
    do {
        ...
        if (lcount) {
            ...
            do {
                if ((lzi >> 16) >= *lpz)
                    if (*lptex != 0xdeafbeef) { // transparent pixels in alias models
                        *lpdest = ((byte *)acolormap)[*lptex + (llight & 0xFF00)];
                        *lpz = lzi >> 16;
                    }
                lpdest++;
...
================
*/
extern cvar_t sw_stipplealpha; // Edited
void D_RasterizeAliasPolySmooth(void)
{
...
//
//	D_PolysetCalcGradients (r_affinetridesc.skinwidth); // transparencies - removed
    // transparencies - begin
#if	id386
    if (!use_c_renderer) {
...
#if	id386
    if (!use_c_renderer) { // transparencies
...
// count field
    ...
    */	// EF_CELSHADING - end
    ...
    // EF_SHADOW - end
    else if (currententity->alpha == 1.0f) { // transparencies
        D_PolysetDrawSpans8(a_spans);
    }
    // transparencies - begin
    else if (!alphamap || sw_stipplealpha.value) {
        D_PolysetDrawSpans8_Stippled(a_spans);
    } else if (currententity->alpha < 0.5f) {
