...
/* // CD player in menu - removed - begin
static qboolean cdValid = false;
static qboolean	playing = false;
static qboolean	wasPlaying = false;
*/ // CD player in menu - removed - end
static qboolean	initialized = false;
//static qboolean	enabled = false; // CD player in menu - removed
//static qboolean playLooping = false; // CD player in menu - removed
static float	cdvolume;
static byte 	remap[100];
static byte		cdrom;
//static byte		playTrack; // CD player in menu - removed
//static byte		maxTrack; // CD player in menu - removed
cvar_t cd_enabled = {"cd_enabled", "1", true}; // Edited
...
void CDAudio_Play(byte track, qboolean looping)
{
    ...
    if (!cd_enabled.value || !initialized) { // CD player in menu - edited
        return;
    }
...
static void CD_f(void)
{
    ...
    if (Q_strcasecmp(command, "on") == 0) {
        Cvar_SetValue("cd_enabled", 1); // CD player in menu
        return;
    }
    ...
    if (Q_strcasecmp(command, "off") == 0) {
        ...
        Cvar_SetValue("cd_enabled", 0); // CD player in menu
        return;
    }
    ...
    // moved the "eject" command here so empty drives can be ejected
    if (Q_strcasecmp(command, "eject") == 0) {
        ...
        CDAudio_Eject();
        cdValid = false;
        maxTrack = 0; // CD player in menu
        return;
    }

    if (Q_strcasecmp(command, "close") == 0) {
        CDAudio_CloseDoor();
        Cbuf_AddText("wait;cd reset\n");  // CD player in menu - auto-update CD status after closing drive door
        return;
    }
...
{
    if (!cd_enabled.value || !initialized) { // CD player in menu
        return;
    }

    if (bgmvolume.value != cdvolume) {
        CD_SetVolume(bgmvolume.value * 0xffff); // Hexen 2
        if (cdvolume && !bgmvolume.value) { // just muted
            CDAudio_Pause();
        } else if (!cdvolume && bgmvolume.value) { // Edited
            CDAudio_Resume();
        }
...
int CDAudio_Init(void)
{
    ...
    static qboolean cd_initialized; // Windows XP fix
    // CD player in menu - begin
    cdValid = false;
    playing = false;
    wasPlaying = false;
    playLooping = false;
    // CD player in menu - end
...
void CDAudio_Shutdown(void)
{
    ...
    if (CD_ID != -1) { // Edited
        auxSetVolume(CD_ID,CD_OrigVolume);    // Edited
    }
