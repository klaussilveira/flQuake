...
// 2001-11-15 DarkPlaces general builtin functions by LordHavoc  end
    ...
    "DP_SND_DIRECTIONLESSATTNNONE",	// sounds with no attenuation full volume on both sides?
...
#endif
        //yaw = /*(int)*/ (atan2(value1[1], value1[0]) * 180 / M_PI); // 16-bit angles - edited
        if (yaw < 0) {
            yaw += 360;
        }
    }
...
#else
        pitch = (myAtan2(value1[2], forward) * 180 / M_PI);
#endif
        //pitch = /*(int)*/ (atan2(value1[2], forward) * 180 / M_PI); // 16-bit angles - edited
        if (pitch < 0) {
            pitch += 360;
        }
    }
...
cvar_t	sv_aim_h = {"sv_aim_h", "1", true}; // horizontal autoaim
#else
cvar_t	sv_aim_h = {"sv_aim_h", "0.98", true}; // horizontal autoaim
#endif
cvar_t	sv_aim = {"sv_aim", "0.93", true}; // saving aim value
void PF_aim(void)
{
...
// try all possible entities
    ...
    // horizontal autoaim - begin
    if (sv_aim_h.value < 1) {
        bestdist = sv_aim_h.value;
    } else
        // horizontal autoaim - end
    {
...
extern int current_protocol; // QC Alpha Scale Glow
