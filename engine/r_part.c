...
void R_ParticleExplosion(vec3_t org)
{
    ...
    for (i=0 ; i<1024 ; i++) {
        ...
        p->die = cl.time + 5;
        p->start_time = cl.time-1.4; // Edited
        p->color = ramp1[0];
        p->ramp = rand()&3;
        if (i & 1) {
            p->type = pt_explode;
...
void R_LavaSplash(vec3_t org)
{
    ...
    for (k=0 ; k<4 ; k++) // edited & moved here
        for (i=-16 ; i<16 ; i++)
            for (j=-16 ; j<16 ; j++) {
                if (!free_particles) {
                    return;
                ...
                p->start_time = cl.time+1.6; // Edited
                p->color = 224 + (rand()&7);
                p->type = pt_slowgrav;
                ...
                VectorNormalize(dir);
                vel = 150 + (rand()&63);//50 + (rand()&63); // edited
                VectorScale(dir, vel, p->vel);
            }
}
...
void R_RocketTrail(vec3_t start, vec3_t end, int type)
{
    ...
    VectorScale(vec, dec, vec);  // Edited
...
#endif
        case pt_slowgrav:
            p->vel[2] -= grav*4; // edited
            break;
        }
//		if (p->die == -1) p->start_time = -2; // Edited
    }
