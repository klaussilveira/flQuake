...
cvar_t snd_stereo = {"snd_stereo", "1", true}; // Edited
cvar_t snd_swapstereo = {"snd_swapstereo", "0", true}; // Edited
...
void S_Startup(void)
{
    ...
    if (!fakedma) {
        ...
        if (!rc) {
            ...
            extern int desired_speed; // Edited
            if (desired_speed) { // Edited
...
void S_Init(void)
{
    ...
    Cvar_RegisterVariable(&snd_swapstereo); // Edited
    Cvar_RegisterVariable(&snd_stereo); // Edited
...
// anything coming from the view entity will allways be full volume
    if (ch->entnum == cl.viewentity || ch->dist_mult == 0) { // edited
...
// calculate stereo seperation and distance attenuation
    ...
    if (shm->channels == 1 || !snd_stereo.value) { // edited
        rscale = 1.0;
        lscale = 1.0;
    }
    // begin
