...
//int             static_registered = 1;  // only for startup check, then set // removing pop.lmp check - removed
...
/* // removing pop.lmp check - removed - begin
...
*/ // removing pop.lmp check - removed - end
...
void MSG_WriteAngle(sizebuf_t *sb, float f)
{
    ...
    if (current_protocol == QB_PROTOCOL) {
        ...
        // 16-bit angles - end
    {
        MSG_WriteByte(sb, (int)(f*256.0/360.0) & 255);    // improved 8-bit angles - edited
    }
}
    ...
    for (; search ; search = search->next) {
        ...
        if (search->pack) {
            ...
            for (i=0 ; i<pak->numfiles ; i++)
                if (!Q_strcasecmp(pak->files[i].name, filename)) { // fixed
                    // found it!
...
//
    for (i=0 ; i<64 ; i++) { // edited
        sprintf(pakfile, "%s/pak%i.pak", dir, i);
        pak = COM_LoadPackFile(pakfile);
        if (!pak) {
            continue;    //break; // edited
        }
        search = Hunk_Alloc(sizeof(searchpath_t));
