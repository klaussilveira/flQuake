...
void Mod_LoadTextures(lump_t *l)
{
    ...
    for (i=0 ; i<m->nummiptex ; i++) {
        ...
        tx = Hunk_AllocName(sizeof(texture_t) +pixels*sizeof(pixel_t), loadname);
        ...
        tx->height = mt->height;
        for (j=0 ; j<MIPLEVELS ; j++) {
            tx->offsets[j] = (mt->offsets[j]- sizeof(miptex_t))*sizeof(pixel_t) + sizeof(texture_t) ;
        }
        // the pixels immediately follow the structures
        if (sizeof(pixel_t)==4) {
            pixel_t* 	pDst	=	(pixel_t*)(tx+1);
            byte* 		pSrc	=	(byte*)(mt+1);
            byte*		pal		=	pSrc + ((mt->width * mt->height * 85) >> 6) + 2;
            int	a;
            if (loadmodel->bspversion == BSPVERSION_HL) {
                for (a=0; a<pixels; a++) {
                    int r,g,b;
                    r		=	pal[pSrc[a]*3];
                    g		=	pal[pSrc[a]*3+1];
...
void Mod_LoadLighting(lump_t *l)
{
    ...
    /*
    	if (loadmodel->bspversion == BSPVERSION_HL)
    	{
    		...
    		for (i=0; i<l->filelen; i+=3)
    		{
    			...
    			out = grayscale > 255?255:grayscale<0?0:(byte)grayscale;
    ...
    */
    if (loadmodel->bspversion == BSPVERSION_HL) {
        int a;
        byte* pSrc	=	mod_base + l->fileofs;
        int Size	=	l->filelen/3;
        loadmodel->lightdata = Hunk_AllocName(Size, loadname);
        for (a=0; a<Size; a++) {
            loadmodel->lightdata[a]	=	(pSrc[a*3]*4+pSrc[a*3+1]*5+pSrc[a*3+2]*1)/10;
        }
        LightmapScaledown	=	3;
    } else {
        loadmodel->lightdata = Hunk_AllocName(l->filelen, loadname);
...
//	out = Hunk_AllocName ( count*sizeof(*out), loadname);
    out = Hunk_AllocName((count+8)*sizeof(*out), loadname);   // skyboxes - extra for skybox
...
//	out = Hunk_AllocName ( (count + 1) * sizeof(*out), loadname);
    out = Hunk_AllocName((count + 13) * sizeof(*out), loadname);   // skyboxes - extra for skybox
...
void Mod_LoadTexinfo(lump_t *l)
{
    ...
    count = l->filelen / sizeof(*in);
//	out = Hunk_AllocName ( count*sizeof(*out), loadname);
    out = Hunk_AllocName((count+6)*sizeof(*out), loadname);   // skyboxes - extra for skybox
...
void Mod_LoadSurfedges(lump_t *l)
{
    ...
    count = l->filelen / sizeof(*in);
//	out = Hunk_AllocName ( count*sizeof(*out), loadname);
    out = Hunk_AllocName((count+24)*sizeof(*out), loadname);   // skyboxes - extra for skybox
...
int Mod_FindExternalVIS(loadedfile_t *brush_fileinfo)
{
    ...
    char	vispathname[MAX_OSPATH]; // Edited
    int		fhandle;
    int		len, i, pos;
    searchpath_t	*s_vis;
    vispatch_t	header;
    ...
    if (external_vis.value) {
        ...
        if (fhandle == -1) {	// check for a .VIS file with map's directory name (e.g. ID1.VIS)
            //	strcpy(visfilename, "maps/"); // removed
            //	strcat(visfilename, COM_SkipPath(brush_fileinfo->path->filename)); // removed
...
void Mod_LoadBrushModel(model_t *mod, void *buffer, loadedfile_t *brush_fileinfo)	// 2001-09-12 .ENT support by Maddes
{
    ...
    if (mod->bspversion!=BSPVERSION_Q1 && mod->bspversion!=BSPVERSION_HL) {
        Con_Printf("Mod_LoadBrushModel: %s has wrong version number (%i should be %i (Quake) or %i (HalfLife))", mod->name, mod->bspversion, BSPVERSION_Q1, BSPVERSION_HL);
                          ...
                          trivertx_t *pbboxmin, trivertx_t *pbboxmax, aliashdr_t *pheader, char *name)
{
    ...
    *pframeindex = (byte *)paliasgroup - (byte *)pheader;
    ...
    for (i=0 ; i<numframes ; i++) {
        paliasgroup->frames[i].numframes = numframes; // model interpolation
...
void * Mod_LoadAliasSkin(void * pin, int *pskinindex, int skinsize,aliashdr_t *pheader)
{
    ...
    *pskinindex = (byte *)pskin - (byte *)pheader;
    ...
    pusskin = (pixel_t*)pskin;
    for (i=0 ; i<skinsize ; i++) {
        pusskin[i] = d_8to24table[pinskin[i]];
    }
...
void * Mod_LoadSpriteFrame(void * pin, mspriteframe_t **ppframe)
{
    ...
    *ppframe = pspriteframe;
    ...
    ppixout = (pixel_t *)&pspriteframe->pixels[0];
